...
namespace :bot do
  desc "Rake task that automates bot behavior"
  task :hype do
    users = User.all.sort { |a,b| a.hypes.count <=> b.hypes.count }
    user = users.first
    user.hypes << Hype.create
    user.save
    redd_user = user.reddit_user
    compliment = user.hypes.last.compliment
    HypeBot.random_post(redd_user, compliment)
  end
end
